...
namespace StableDiffusionSharp.ModelLoader
{
	internal class PickleLoader
	{
		...
		private static List<TensorInfo> ReadTensorsInfoFromFile(string fileName)
		{
			...
			TensorInfo tensor = new TensorInfo() { FileName = fileName, Offset = { 0 } };
				...
				switch (opcode)
				{
					...
					case (byte)'R': // REDUCE         = b'R'   # apply callable to argtuple, both on stack
						if (deepth == 1)
						{
							...
							tensor = new TensorInfo() { FileName = fileName, Offset = { 0 } };
							readStrides = false;
							binPersid = false;
						}
						break;
			...
			TensorInfo metaTensor = tensors.Find(x => x.Name.Contains("_metadata"));
			if (metaTensor != null)
			{
