...
namespace StableDiffusionSharp.Modules
{
	internal class UNet
	{
		...
		internal class SpatialTransformer : Module<Tensor, Tensor, Tensor>
		{
			...
			protected override void Dispose(bool disposing)
			{
				if (disposing)
				{
					...
					proj_in?.Dispose();
					proj_out?.Dispose();
					foreach (var block in transformer_blocks)
					{
						block.Dispose();
					}
				}
		...
		internal class SDUnet : Diffusion
		{
			private class UNet : Module<Tensor, Tensor, Tensor, Tensor>
			{
				...
				protected override void Dispose(bool disposing)
				{
					if (disposing)
					{
						...
						middle_block?.Dispose();
						foreach (var block in output_blocks)
						{
							block.Dispose();
						}
						@out?.Dispose();
			...
			public SDUnet(int model_channels, int in_channels, int num_heads = 8, int context_dim = 768, float dropout = 0.0f, bool use_timestep = true, Device? device = null, ScalarType? dtype = null)
			{
		...
		internal class SDXLUnet : Diffusion
		{
			...
			public SDXLUnet(int model_channels, int in_channels, int num_heads = 20, int context_dim = 2048, int adm_in_channels = 2816, float dropout = 0.0f, bool use_timestep = true, Device? device = null, ScalarType? dtype = null)
			{
		...
		internal abstract class Diffusion : Module<Tensor, Tensor, Tensor, Tensor, Tensor>
		{
			public Diffusion() : base(nameof(Diffusion))
			{
